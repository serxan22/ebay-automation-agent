...
create table if not exists public.automation_settings (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade unique,
  daily_listing_limit integer not null default 5 check (daily_listing_limit between 0 and 250),
  min_profit_amount numeric(12, 2) not null default 5,
  min_margin_percentage numeric(5, 2) not null default 25,
  max_shipping_days integer not null default 5 check (max_shipping_days >= 0),
  ...
  risk_tolerance integer not null default 80 check (risk_tolerance between 0 and 100),
