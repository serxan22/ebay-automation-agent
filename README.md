...
Use `EBAY_RUNAME` for the eBay Developer portal Redirect URL name/RuName. Keep `EBAY_REDIRECT_URI` as the real application callback URL, for example `https://your-ngrok-url/ebay/callback`.

Vercel production values for this deployment:

```bash
APP_URL=https://ebay-automation-agent.vercel.app
EBAY_REDIRECT_URI=https://ebay-automation-agent.vercel.app/ebay/callback
...
```text
Privacy policy URL: https://ebay-automation-agent.vercel.app/privacy
Auth accepted URL: https://ebay-automation-agent.vercel.app/ebay/callback
Auth declined URL: https://ebay-automation-agent.vercel.app/ebay/declined
```
...
8. If Vercel logs show `/ebay/callback` or `[ebay_oauth_callback_hit]`, the app callback is being reached.
...
10. In that case, create a new eBay Redirect URL/RuName, set Auth accepted URL exactly to `https://ebay-automation-agent.vercel.app/ebay/callback`, and update `EBAY_RUNAME` in Vercel.
...
4. Click `Setup location` to create/check the stable `default-sandbox-location` warehouse inventory location.
...
2. Open `https://ebay-automation-agent.vercel.app/dashboard/settings`.
...
7. Click `Setup location`.
8. Create or revise a listing draft until readiness is green, then approve it.
9. Click `Publish sandbox`.

Manual sandbox setup may still be required inside eBay: sandbox seller accounts often need Business Policies enabled and payment, return, and fulfillment policies created before policy sync can succeed.
