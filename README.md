...
Use `EBAY_RUNAME` for the eBay Developer portal Redirect URL name/RuName. Keep `EBAY_REDIRECT_URI` as the real application callback URL, for example `https://your-ngrok-url/api/ebay/oauth/callback`.

Vercel production values for this deployment:

```bash
APP_URL=https://ebay-automation-agent.vercel.app
EBAY_REDIRECT_URI=https://ebay-automation-agent.vercel.app/api/ebay/oauth/callback
...
Auth accepted URL: https://ebay-automation-agent.vercel.app/api/ebay/oauth/callback
Auth declined URL: https://ebay-automation-agent.vercel.app/dashboard/settings?ebay=declined
```
